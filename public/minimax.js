(function(){
	...
	*/
	var heuristic = function(state, maximizingPlayer){
		...
		var sign = (maximizingPlayer == 'o') ? -1 : 1;

		function score () {
			if (linesOfLengthFourForX) return 100
			if (linesOfLengthFourForO) return -100
			var sumOfX = linesOfLengthTwoForX + linesOfLengthThreeForX
			var sumOfO = linesOfLengthTwoForO + linesOfLengthThreeForO
	...
	*/
	var minimax = function(state, depth, maximizingPlayer){
		...
		//Your code here.
		if (depth === 0 || state.legalMoves.length === 0 || possibleStates.length === 0) return heuristic(state, state.maximizingPlayer)
		...
		for (var i = 0; i < possibleStates.length; i++) {
			numbers.push(minimax(possibleStates[i], depth--, state.maximizingPlayer ))
		}

		var decision

		if (currentPlayer == state.maximizingPlayer) decision = Math.min.apply(null, numbers);
		else decision = Math.max.apply(null, numbers)
